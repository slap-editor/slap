...
var configFile = path.join(baseDir, package.name + '.hjson');
var config = Hjson.parse(fs.readFileSync(configFile, 'utf-8'));
var opts = util.parseOpts(rc(package.name, config, null, function(content) {
  if (/^\s*{|^\s*#/.test(content)) return Hjson.parse(content);
    ...
    if (!opts._.length) { // if no files are passed
      ...
      if (!opts.config) { // first run without a file passed
        slap.open(path.join(baseDir, 'README.md'), true)
          ...
          .done();
        fs.createReadStream(path.join(__dirname, '..', 'default-config.hjson')).pipe(fs.createWriteStream(path.join(userDir, 'config')));
      }
    }
