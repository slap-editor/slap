...
module.exports = function (options) {
  ...
  return Slap.getUserDir().catch(Promise.resolve()).then(function (userDir) {
    var loggerOpts = _(userDir ? {file: path.resolve(userDir, package.name+'.log')} : {}).merge(opts.logger).toObject();
    opts = _(opts).merge({logger: loggerOpts, editor: {logger: loggerOpts}}).toObject();
    util.logger(opts.logger);

    util.logger.info("loading...");
    util.logger.verbose("configuration:", opts);
    var slap = new Slap(_(opts).merge({screen: new blessed.Screen()}).toObject());
