...
var markupRegExp = /{(\/?)([\w\-,;!#]*)}/g;
Editor._realIndex = function (markup, index) {
  var i = 0;
  while (index > 0) {
    var match = markup.match(markupRegExp);
    ...
    .map(function (line, y) {
      ...
      if (selection && selection.start.y <= y && y <= selection.end.y) {
        ...
        line = line.substring(0, start) +
          self.options.selectStyle + line.substring(start, end) + self.options.selectStyle.replace(markupRegExp, function (match) { return '{!' + match.slice(1); }) +
          line.substring(end);
      }
