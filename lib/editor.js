...
Editor.find = function (text, pattern, start, end) {
  ...
  var match = Editor._getString(textRange).match(pattern);
  if (!match) return;

  var result = start || {x: 0, y: 0};
  
  Editor._getLines(textRange).some(function (line, y) {
...
Editor.prototype.change = function (text, start, end) {
  ...
  var textLines = Editor._getLines(text);
...
Editor.prototype.select = function (start, end) {
  ...
  if (arguments.length) {
    ...
    return {
      ...
      text: Editor.textRange(self.data.lines, selectionBounds[0], selectionBounds[1]).join('')
    };
  }
};
