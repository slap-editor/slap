...
Editor._markupIndex = function (markup, index) {
  ...
  return new Lexer()
    ...
    .addRule(Editor._markupRegExp, function (tag, close, name) {
      i += tag.length;
      if (done) {
        if (!close) { appendTags.push(tag); }
        else {
          var lastTagIndex = appendTags.lastIndexOf('{'+name+'}');
          if (lastTagIndex !== -1) { appendTags.splice(lastTagIndex, 1); }
        }
      }
    })
    .addRule(/[^{}]*/, addText)
...
Editor._closeTags = function (markup) {
  return markup.replace(Editor._markupRegExp, '{!$1$2}', 'g'); // 'g' flag ignored :(
};
...
//    .concat(_.repeat('', self.size().y).toArray())
    ...
    .map(function (line, y) {
      ...
      line = line.replace(Editor._lineRegExp, '');
      if (!line) { return line; } // FIXME: blessed deletes empty lines
      ...
      if (selection && selection.start.y <= y && y <= selection.end.y) {
        ...
        line = line.slice(0, start) +
          selectionStyle + line.slice(start, end) + Editor._closeTags(selectionStyle) +
          line.slice(end);
      }
