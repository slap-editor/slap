...
Editor.prototype._editorRender = function () {
  ...
  var marginStyle = self.options.style.margin || '';
...
//    .concat(_.repeat('', self.size().y).toArray())
    ...
    .map(function (line, y) {
      ...
      var maxWidthSpaces = _.repeat(' ', Math.max(self.data.maxWidth, size.x)).join('');
      line = line.replace(Editor._lineRegExp, '') + maxWidthSpaces;
      ...
      if (self.options.lineNumberWidth) {
        line = markup.markupLine((maxWidthSpaces + y).slice(-self.options.lineNumberWidth), marginStyle) +
          ' ' + line;
      }
