        ...
        return _({
          ...
          if (directionDistance.indexOf(directionName) === 0) {
            ...
            if (directionName === 'All') {
              startCursor = Coordinate.returnsOrigin();
              self.cursor(Coordinate.returnsInfinity());
              ...
              switch (axis) {
                case 1: // left/right
                  switch (distance) {
                    case '':
                      ...
                      if (action === 'go' && startSelection && Coordinate.linear.cmp(startSelection, startCursor) === direction) {
                ...
                case 2: // up/down
                  switch (distance) {
                    ...
                    case 'Infinity': self.cursor(Coordinate[direction === -1 ? 'returnsOrigin' : 'returnsInfinity']()); break;
                    default: moved = false; break;
                  }
              }
            }
...
Editor.prototype.text = function (text, load) {
  ...
  if (arguments.length) {
    text = text.toString();
    if (load) { self.data.markup = ''; }
    self.emit('text', text, load);
    return self.lines(Editor._splitLines(text));
  } else {
...
Editor.prototype.textRange = function (start, end) {
  start = start || Coordinate.returnsOrigin();
  end = end || Coordinate.returnsInfinity();
...
Editor.prototype._changed = function (text, start, end) {
  this.emit('change', text, start, end);
...
Editor.prototype.find = function (pattern, start, end) {
  var self = this;

  var textRange = self.textRange.apply(self, util.toArray(arguments).slice(1));

  var match = textRange.match(pattern);
  if (!match) return;

  var result = start || {x: 0, y: 0};
  Editor._splitLines(textRange).some(function (line, y) {
...
Editor.prototype.matchingBracket = function (pos) {
  ...
  var lines = Editor._splitLines(start
    ? self.textRange(pos)
    : self.textRange(Coordinate.returnsOrigin(), {x: pos.x + 1, y: pos.y}));
...
//    .concat(_.repeat('', self.size().y).toArray())
    ...
    .map(function (line, y) {
      ...
      line = line.replace(Editor._lineRegExp, '') + _.repeat(' ', Math.max(self.data.maxWidth, size.x)).join('');
