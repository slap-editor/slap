...
Editor._splitLines = function (text) {
  var lines = [];
  var match, line;
  while (match = text.match(Editor._lineRegExp)) {
  ...
  if (paragraphs) {
    ...
    while (paragraphs--) {
      while (true) {
        ...
        if (self._visibleLine(cursor.y).match(/^\s*$/g)) { break; }
      }
    }
  } else {
    cursor.y += count;
...
Editor._markupRegExp = /{(\/?)([\w\-,;!#]*)}/;
Editor._realIndex = function (markup, index) {
  var i = 0;
  while (index > 0) {
    var match = markup.match(Editor._markupRegExp);
