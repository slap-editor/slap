...
Editor.prototype.change = function (text, start, end) {
  ...
  self.emit('change', text, start, end, start.load, oldText);
  self.emit('lines', util.toArray(lines), start.load, oldText);
  self.emit('text', self.text(), start.load, oldText);
  return this;
};
...
Editor.ChangeCommand = Undo.Command.extend({
  ...
  change: function (oldText, newText) {
    ...
    return this.editor.change(newText, this.state.start, {
