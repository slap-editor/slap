...
Editor.prototype._initHighlightProcess = function () {
  ...
  self.highlightProcess = fork(path.join(__dirname, 'highlightProcess.js'), {silent: false});
        ...
        default:
          if (!binding && !key.ctrl && ch) {
            ...
            if (selection.text) {
              ...
              var overwrite = !lastCharacter && !self.data.enterPressed;
              var lastEl = self;
...
Editor.prototype._renderableTabString = function () {
...
Editor.prototype._renderableSpace = function () {
  return !this.options.visibleWhiteSpace
    ? ' '
    : markup.markupLine('\u00b7', this.options.style.whiteSpace);
};
Editor.prototype._renderableLineEnding = function (lineEnding) {
  return !this.options.visibleLineEndings
    ? ''
    : markup.markupLine(lineEnding
        .replace(/\n/g, '\\n')
        .replace(/\r/g, '\\r')
     , this.options.style.whiteSpace);
};
...
Editor.prototype.render = function () {
  ...
  matchingBracket = matchingBracket && self.visiblePos(matchingBracket);
    ...
    .map(function (line, y) {
      ...
      if (matchingBracket) {
        if (y === visibleCursor.y) {
          line = markup.markupLine(line, bracketStyle, visibleCursor.x - x, visibleCursor.x - x + 1);
        }
        if (y === matchingBracket.y) {
          line = markup.markupLine(line, bracketStyle, matchingBracket.x - x, matchingBracket.x - x + 1);
        }
      }

      visibleMatches.forEach(function (match) {
        if (match.start.y <= y && y <= match.end.y) {
          line = markup.markupLine(line, matchStyle,
