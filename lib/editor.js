...
Editor.prototype._initHighlightProcess = function () {
  ...
  self.data.markup = '';
  self.highlightProcess = spawn(path.join(__dirname, 'highlightProcess.js'));

  var stdinJSON = es.stringify();
  stdinJSON.pipe(self.highlightProcess.stdin);
  ...
  self.highlightProcess.stderr.pipe(process.stderr);
