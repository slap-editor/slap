...
Editor.prototype._renderableTabString = function () {
  return !this.options.visibleWhiteSpace
    ...
    : _.repeat('-', this.options.tabSize - 1).join('') + (this.options.tabSize ? '>' : '');
};
Editor.prototype._renderableSpace = function () {
  return !this.options.visibleWhiteSpace ? ' ' : '\u00b7';
};
Editor.prototype._renderableLineEnding = function (lineEnding) {
  return !this.options.visibleLineEndings ? '' : lineEnding.replace(/\n/g, '\\n').replace(/\r/g, '\\r');
};
...
//    .concat(_.repeat('', self.size().y).toArray())
    ...
    .map(function (line, y) {
      ...
      line = line
        .replace(Editor._tabRegExp, markup.markupLine(self._renderableTabString(), style.whiteSpace))
        .replace(/ /g, markup.markupLine(self._renderableSpace(), style.whiteSpace))
        .replace(textUtil._lineRegExp, markup.markupLine(renderableLineEnding, style.whiteSpace))
