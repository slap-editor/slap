...
Editor._addTag = function (tags, tag, close, name) {
...
Editor._markupIndex = function (markup, index) {
  ...
  return new Lexer()
    ...
    .addRule(Editor._markupRegExp, function (tag, close, name) {
      i += tag.length;
      if (done) {
        Editor._addTag.apply(null, [appendTags].concat(util.toArray(arguments)));
      }
    })
    .addRule(/[^{}]*/, addText)
    .setInput(markup)
...
Editor._closeTags = function (markup) {
  return markup.replace(Editor._markupRegExp, '{/$2}', 'g'); // 'g' flag ignored :(
};
Editor._escapeCurlies = function (text) {
  return text.replace(/[{}]/g, function (match) {
    return {'{': '{open}', '}': '{close}'}[match];
  });
};
Editor._getOpenTags = function (text) {
  var openTags = [];
  new Lexer()
    .addRule(/\{(open|close)\}/, function () {})
    .addRule(Editor._markupRegExp, Editor._addTag.bind(null, openTags))
    .addRule(/[^{}]*/, function () {})
    .setInput(text)
    .lex();
  return openTags;
};
Editor._markupLine = function (line, markup, startX, endX) {
  startX = typeof startX !== 'undefined' ? Editor._markupIndex(line, startX) : 0;
  endX = typeof endX !== 'undefined' ? Editor._markupIndex(line, endX) : Infinity;
  return line.slice(0, startX) +
    markup + line.slice(startX, endX) + Editor._closeTags(markup) +
    line.slice(endX);
};
Editor.prototype._markup = function (update) {
  var self = this;

  var markup = self.data.markup;
  if (update || !('markup' in self.data)) {
    var text = self.text();
    var language = self.language();
    if (language) {
      var $ = cheerio.load(Editor._escapeCurlies(hljs.highlight(language, text, true).value)); // Assumes hljs output doesn't use curlies
      ...
      do {
        ...
        var elCount = $('*:not(:has(*))').replaceWith(function () {
          ...
          var tags = ($el.attr('class') || '')
            .split(' ')
            .map(function (className) { return self.options.style[className] || ''; })
            .join('');
          return Editor._markupLine($el.text(), tags);
        }).length;
      } while (lastElCount !== elCount);

      markup = $.root().text();
    } else {
      markup = Editor._escapeCurlies(text);
    }
    self.data.markup = markup;
  }
  return markup;
};
...
Editor.prototype._editorRender = function () {
  ...
  self.setContent(Editor._splitLines(self._markup())
//    .concat(_.repeat('', self.size().y).toArray())
    ...
    .map(function (line, y) {
      ...
      if (selectionStyle && selection.start.y <= y && y <= selection.end.y) {
        ...
        line = Editor._markupLine(line, selectionStyle, startX, endX);
      }

      var markupScrollX = Editor._markupIndex(line, scroll.x);
      return Editor._getOpenTags(line.slice(0, markupScrollX)).join('') + line.slice(
        markupScrollX,
        Editor._markupIndex(line, scroll.x + size.x)
      ) + '{/}';
    })
    .join('\n'));
