...
Editor.prototype.text = function (text, language) {
  ...
  if (arguments.length) {
    return self
      .language(language)
      .change(text.toString(), Coordinate.origin(), Coordinate.infinity());
  } else {
...
Editor.prototype.select = function (start, end) {
  ...
  if (arguments.length) {
    ...
    return {
      ...
      text: textUtil.range(self.data.lines, selectionBounds[0], selectionBounds[1]).join('')
    };
  }
};
...
Editor.ChangeCommand = Undo.Command.extend({
  ...
  undo: function () {
    this
      ...
      .select(this.state.start, this.state.end)
  ...
  change: function (oldText, newText) {
    ...
    var editor = this.editor.change(newText, this.state.start, {
      x: textUtil._getLineSafe(oldLines, Infinity).length + (oldLines.length === 1 && this.state.start.x),
...
Editor.prototype._initChangeStack = function () {
  ...
  self.on('change', function (newText, start, end, load, oldText) {
    ...
    if (load) {
      ...
      self.changeStack.execute(new Editor.ChangeCommand(self, changeState));
    }
  });
  return self;
};
...
Editor.prototype._initHandlers = function () {
  ...
  self.on('cursor', function (cursor) {
    ...
    var minScroll = cursor
      .subtract({x: cursorPadding.left, y: cursorPadding.top});
    var maxScroll = cursor
      .subtract(self.size())
      .add({x: cursorPadding.right, y: cursorPadding.bottom})
      .add({x: 1, y: 1});
...
Editor.prototype._updateCursor = function () {
  var self = this;
  var cursorOnScreen = Coordinate(self.pos())
    .add(self.visiblePos(self.cursor()))
...
Editor.prototype.render = function () {
  ...
  var visibleSelection = {
    start: self.visiblePos(selection.start),
    end: self.visiblePos(selection.end)
  };

  var style = self.options.style;
  var selectionStyle = style.selection;
  var currentLineStyle = style.currentLine;
  var bracketStyle = matchingBracket ? style.matchingBracket : style.mismatchedBracket;

  self.setContent(textUtil.splitLines(self.data.markup)
//    .concat(_.repeat('', self.size().y).toArray())
    .slice(scroll.y, scroll.y + size.y)
    .map(function (line, y) {
      y += scroll.y;

      line = textUtil.stripLine(line)
        .replace(Editor._tabRegExp, _.repeat(' ', self.options.tabSize).join(''))
        .replace(Editor._nonprintableRegExp, '\ufffd');
      line += _.repeat(' ', Math.max(self.data.maxWidth, size.x)).join('');

      if (selectionStyle && visibleSelection.start.y <= y && y <= visibleSelection.end.y) {
        var startX = y === visibleSelection.start.y ? visibleSelection.start.x : 0;
        var endX = y === visibleSelection.end.y ? visibleSelection.end.x : Infinity;
        line = markup.markupLine(line, selectionStyle, startX, endX);
      }

      if (matchingBracket) {
        if (y === visibleCursor.y) {
          line = markup.markupLine(line, bracketStyle, visibleCursor.x, visibleCursor.x + 1);
        }
        if (y === matchingBracket.y) {
          line = markup.markupLine(line, bracketStyle, matchingBracket.x, matchingBracket.x + 1);
        }
      }

      if (self === self.screen.focused && currentLineStyle && y === visibleCursor.y) {
