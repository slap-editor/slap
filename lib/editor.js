...
Editor._markupLine = function (line, markup, startX, endX) {
  startX = Editor._markupIndex(line, startX);
  endX = Editor._markupIndex(line, endX);
  return line.slice(0, startX) +
    markup + line.slice(startX, endX) +
    '{/}' + Editor._getOpenTags(line.slice(0, endX)).join('') +
    line.slice(endX);
};
      ...
      do {
        ...
        var elCount = $('*:not(:has(*))').replaceWith(function () {
          ...
          var tags = ($el.attr('class') || '')
            ...
            .join('');
          return tags + $el.text() + Editor._closeTags(tags);
        }).length;
