    ...
    if (mouseData.action === 'wheeldown' || mouseData.action === 'wheelup') {
      ...
      if (self.data.mouseDown) {
        if (lastClick && Coordinate.linear.cmp(lastClick, mouse) === 0) {
          ...
          var currentWord = word.current(line, mouse.x);
          self.select(
            {x: word.prev(line, mouse.x).index, y: mouse.y},
            {x: currentWord.index + currentWord[0].length, y: mouse.y}
          );
        } else {
          self.cursor(mouse);
        }
...
Editor.prototype._markup = function (update) {
  ...
  var storedMarkup = self.data.markup;
  if (update || !('storedMarkup' in self.data)) {
    var text = self.text();
    var language = self.language();
    if (language) {
      var $ = cheerio.load(markup.escapeCurlies(hljs.highlight(language, text, true).value)); // Assumes hljs output doesn't use curlies
      ...
      do {
        ...
        var elCount = $('*:not(:has(*))').replaceWith(function () {
          ...
          var tags = ($el.attr('class') || '')
            ...
            .join('');
          return markup.markupLine($el.text(), tags);
        }).length;
      } while (lastElCount !== elCount);

      storedMarkup = $.root().text();
    } else {
      storedMarkup = markup.escapeCurlies(text);
    }
    self.data.markup = storedMarkup;
  }
  return storedMarkup;
};
