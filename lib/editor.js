...
//    .concat(_.repeat('', self.size().y).toArray())
    ...
    .map(function (line, y) {
      ...
      line = textUtil.stripLine(line).replace(Editor._nonprintableRegExp, '\ufffd');
      ...
      if (matchingBracket) {
        if (y === cursor.y) {
          line = markup.markupLine(line, bracketStyle, cursor.x, cursor.x + 1);
        }
      ...
      if (self === self.screen.focused && currentLineStyle && y === cursor.y) {
