...
function Editor (opts) {
  ...
  self
    .toggleInsertMode()
    .text('')
    .cursor(0, 0).preferredCursorX(true)
    .scroll(0, 0)
    ._initHandlers();
}
  ...
  self.on('mouse', function (mouseData) {
    ...
    if (mouseData.action === 'wheeldown' || mouseData.action === 'wheelup') {
      ...
      if (self.data.mouseDown) {
        self.cursor(mouse).preferredCursorX(true);
      }
...
Editor.prototype.visiblePos = function (pos) {
  pos = coordinate.apply(this, arguments);
...
Editor.prototype.preferredCursorX = util.getterSetter('preferredCursorX', function (x) {
  return this.realPos(x, this.cursor().y).x;
}, function () {
  return this.visiblePos(this.cursor()).x;
});
Editor.prototype.moveCursorVertical = function (count, paragraphs) {
  ...
  self
    .cursor(cursor) // updates vertical position so .preferredCursorX knows correct line
    .cursor({
      x: Math.max(0, cursor.y < self.data.lines.length
        ? self.preferredCursorX()
    ...
    while (true) {
      if (-count > cursor.x) {
        ...
        if (count > restOfLineLength) {
          ...
          // Same line
          cursor.x += count;
          self.cursor(cursor).preferredCursorX(true);
          break;
        }
      }
    }
...
Editor.prototype._updateCursor = function () {
  var self = this;
  var cursorOnScreen = coordinate.add(self.pos(), coordinate.subtract(self.visiblePos(this.cursor()), self.scroll()));
