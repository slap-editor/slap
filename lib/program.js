...
Program.prototype.prompt = function (prompts, cb) {
  ...
  function done (submit) {
    ...
    self.parent.lockKeys = true;
    process.nextTick(function () { self.parent.lockKeys = false; });
  }
...
Program.prototype._initHandlers = function () {
    ...
    switch (util.getBinding(self.options.bindings, key)) {
      ...
      case 'find':
        self.prompt("find (/.*/ for regex)", function (results) {
          if (results) {
            pattern = results[0];
            var regExpMatch = pattern.match(Program._regExpRegExp);
            pattern = regExpMatch
            ...
            if (pos) {
              ...
              if (pos) {
                if (Coordinate.linear.cmp(pos, self.editor.cursor()) === 0) {
                  self.message(markup.markupLine("this is the only occurrence", style.info));
                }
                self.editor.cursor(pos);
              } else {
                self.message(markup.markupLine("no matches", style.warning));
              }
            }
          }
        });
  ...
  ['change', 'cursor', 'insertMode'].forEach(function (evt) {
    self.editor.on(evt, function () { self.parent.render(); });
  });
...
Program.prototype.render = function () {
  ...
  var left = ' \u270b ';
  var filename = markup.escapeCurlies(self.filename || 'new file');
  if (self.editor.changeStack.dirty()) {
    filename = markup.markupLine(filename+'*', style.changed);
  }
  left += filename;
