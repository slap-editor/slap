...
Program.prototype._initHandlers = function () {
  ...
  self.screen.key('C-f', function () {
    self.form("find", function (pattern) {
      pattern = pattern[0];
...
//  self.on('message', function () { self.render(); });
...
Program.prototype.form = function (prompts, cb) {
  var self = this;
  ...
  var labelWidth = _(prompts).pluck('length').max() + 1;
  var label = new blessed.Box({
    parent: self._form,
    tags: true,
    content: prompts.map(function (prompt) {
      return markup.markupLine((prompt + _.repeat(' ', labelWidth)).slice(0, labelWidth), '{white-bg}{blue-fg}');
    }).join('\n'),
    top: 0,
    left: 0,
    width: labelWidth,
...
Program.prototype.closeForm = function () {
  var self = this;

  if (self._form) {
    self.editor.focus();
    self.editor.bottom = 0;
    self.screen.remove(self._form);
    self._form = null;
    self.render();
  }
