...
Slap.prototype.open = function (givenPath) {
  ...
  return fs.readFileAsync(givenPath).then(function (data) {
    self.editor.text(data, givenPath.split('.').pop());
    return self.path(givenPath);
  });
};
  ...
  return new Promise(function (resolve, reject) {
    ...
    function done (submit) {
      ...
      process.nextTick(function () {
        self.lockKeys = false;
        resolve(submit ? _(inputs).pluck('value').toArray() : null);
      });
    }
...
Slap.prototype.render = function () {
  ...
  if (message) {
    right.unshift(markup.markupLine(message, self.data.blink ? style.blink : ''));
    if (self.options.blinkRate && !self.data.messageBlinkTimer) {
      (function blink () {
        self.data.messageBlinkTimer = setTimeout(function () {
          self.data.blink = !self.data.blink;
          self.render();
          blink();
    ...
    clearTimeout(self.data.messageBlinkTimer);
    self.data.messageBlinkTimer = false;
    self.data.blink = false;
  }
  right = right.join('  ') + ' ';
