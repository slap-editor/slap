...
Slap.prototype.save = function (path) {
  var self = this;
  path = path || self.path();
  if (!path) { self.message("could not save file", 'error'); return; }
    ...
    switch (util.getBinding(self.options.bindings, key)) {
      ...
      case 'save':
        if (self.path()) { self.save(); }
          ...
          self.prompt("save as", function (results) { self.save(results); });
        }
        break;
    }
  });
...
Slap.prototype.render = function () {
  ...
  var left = ' \u270b ';
  var path = markup.escapeCurlies(self.path() || 'new file');
  if (self.editor.changeStack.dirty()) {
