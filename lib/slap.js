...
Slap.prototype._initHandlers = function () {
  ...
  self.on('element keypress', function (el, ch, key) {
    ...
    switch (util.getBinding(self.options.bindings, key)) {
      ...
      case 'save': (self.path() ? self.save() : self.saveAs()).done(); break;
      case 'saveAs': self.saveAs().done(); break;
  ...
  ['resize', 'save', 'path', 'insertMode', 'message', 'blink'].forEach(function (evt) {
...
Slap.prototype.render = function () {
  ...
  if (!self.insertMode()) right.unshift(markup.markupLine('OVR', '{red-bg}{white-fg}'));
