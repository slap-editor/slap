...
function Editor (opts) {
  ...
  // Check if fallbackEncoding is defined and supported by iconv, if not, use a static fallback
  if (self.options.fallbackEncoding == null || !iconv.encodingExists(self.options.fallbackEncoding)) self.options.fallbackEncoding = 'UTF-8';
...
Editor.prototype.encoding = util.getterSetter('encoding', null, null);
Editor.prototype.getEncoding = function (path) {
  var self = this;
  if (self.options.encoding == null){
    var encoding = chardet.detectFileSync(path);
  } else {
    var encoding = self.options.encoding;
  }
  if (!iconv.encodingExists(encoding)) encoding = self.options.fallbackEncoding;
  return encoding;
};
Editor.prototype.open = function (path) {
  var self = this;
  path = util.resolvePath(path);
  var encoding = self.getEncoding(path);
...
Editor.prototype.save = Promise.method(function (path) {
  ...
  var text = self.text();
  return fs.writeFileAsync(path, text, {encoding: encoding, flags: 'w'})
    .then(function () {
