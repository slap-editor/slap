...
function Editor (opts) {
  ...
  var _updateContent = self._updateContent;
...
Editor.prototype.open = Promise.method(function (path) {
  ...
  return chardet.detectFileAsync(path) // return Promise.resolve(chardet.detectFileSync(path))
    ...
    .then(function () { return new Promise(process.nextTick); }) // ensures textBuf path-changed is triggered in _initHandlers
    ...
    .catch(function (err) {
      switch ((err.cause || {}).code) {
        ...
        case 'ENOENT':
          self.checkpoint = self.textBuf.createCheckpoint();
          self.screen.render();
          break;
        default: throw err;
      }
...
Editor.prototype.save = Promise.method(function (path) {
  var self = this;
  return (path ? Promise.resolve(self.textBuf.saveAs(util.resolvePath(path))) : self.textBuf.save())
    .then(function () {
...
Editor.prototype.delete = function (range) {
  var self = this;
  if (!range) {
    self.textBuf.delete(self.selection.getRange());
...
Editor.prototype.bufferPos = function () {
  return new Point(this.buffer.top + this.buffer.itop, this.buffer.left + this.buffer.ileft);
};

Editor.prototype.bufferSize = function () {
  return new Point(this.buffer.height - this.buffer.iheight, this.buffer.width - this.buffer.iwidth);
};

Editor.prototype._requestHighlight = function () {
  var self = this;
  if (self.options.highlight) {
    getHighlightClient.done(function (highlightClient) {
...
Editor.prototype._initHighlighting = function () {
  ...
  self.data.highlight = {ranges: [], revision: 0, bucket: getHighlightClient.getBucket()};
  ...
  getHighlightClient.done(function (highlightClient) {
    ...
    highlightClient.once('message', function highlight (data) {
      ...
      if (data.bucket === self.data.highlight.bucket && data.revision >= self.data.highlight.revision) {
        ...
        self.textBuf.findMarkers({type: 'syntax'}).forEach(function (marker) { marker.destroy(); });
...
Editor.prototype._clipScroll = function (pos) {
  var self = this;
  ...
  if (pos) {
    ...
    self.scroll = new Point(
      Math.max(Math.min(maxScroll.row,    self.scroll.row),    minScroll.row),
      Math.max(Math.min(maxScroll.column, self.scroll.column), minScroll.column));
  }
  self.scroll = new Point(
    Math.max(0, Math.min(self.scroll.row, self.textBuf.getLineCount() - size.row)),
    Math.max(0, self.scroll.column));
  return self;
};
Editor.prototype._initHandlers = function () {
        ...
        return [
          ...
          if (directionDistance.indexOf(direction.name) === 0) {
            ...
            if (direction.name === 'All') {
              ...
              if (action !== 'delete' || !selectionDirection) {
  ...
  self.on('mouse', function (mouseData) {
    if (mouseData.action === 'wheeldown' || mouseData.action === 'wheelup') {
      ...
      var mouse = self.realPos(new Point(mouseData.y, mouseData.x)
        .translate(self.bufferPos().negate())
      ...
      if (mouseData.action === 'mousedown') {
        ...
        if (lastClick && lastClick.isEqual(mouse)) {
          self.data.lastClick = null;
          ...
          self.selection.setRange(new Range(new Point(mouse.row, startX), new Point(mouse.row, endX)));
        } else {
          if (!self.data.mouseDown) self.selection.clearTail();
          self.data.mouseDown = true;
        }
      }
      if (self.data.mouseDown) {
        self.selection.setHeadPosition(mouse);
        self.selection.plantTail();
      }
  ...
  selection.on('changed', function () {
    ...
    var selectionText = self.textBuf.getTextInRange(selectionRange);
    var line = self.lineWithEndingForRow(selectionRange.end.row);

    self.textBuf.findMarkers({type: 'match'}).forEach(function (marker) { marker.destroy(); });
    if (selectionRange.isSingleLine() && selectionText.match(/^[\w.-]+$/)
     && (line[selectionRange.start.column - 1] || ' ').match(/\W/)
     && (line[selectionRange.end.column] || ' ').match(/\W/)) {
...
Editor.prototype._updateCursor = function () {
  ...
  var bufferPos = self.bufferPos();
  var cursorOnScreen = self.visiblePos(self.selection.getHeadPosition())
    .translate(bufferPos)
    .translate(self.scroll.negate());
  if (new Range(new Point(0, 0), self.bufferSize().translate(new Point(-1, -1))).containsPoint(cursorOnScreen.translate(bufferPos.negate())) && self === self.screen.focused) {
    self.screen.program.move(cursorOnScreen.column, cursorOnScreen.row);
...
Editor.prototype._updateContent = function () {
  ...
  var size = self.isAttached() ? self.bufferSize() : new Point(0, 0); // little hack
  ...
  textUtil.splitLines(blessed.escape(self.textBuf.getTextInRange({
    ...
    var markupColumn = markup.index(line, column);
    line = markup.getOpenTags(line.slice(0, markupColumn)).join('') +
      markup(line.slice(markupColumn, markup.index(line, column + size.column)) + _.repeat(' ', size.column).join(''), defaultStyle);

    self.textBuf.findMarkers({intersectsRow: row}).forEach(function (marker) {
      ...
      if (range.intersectsRow(row)) {
        ...
        switch (marker.properties.type) {
          ...
          case 'match': markerStyle = matchStyle; break;
