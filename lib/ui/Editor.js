...
Editor._spawnHighlightDaemon = function () {
  var oldMessageListeners = Editor.highlightDaemon ? Editor.highlightDaemon.listeners('message') : [];
  Editor.highlightDaemon = fork(path.join(__dirname, '..', 'highlightDaemon.js'), {silent: false});
  Editor.highlightDaemon.on('exit', Editor._spawnHighlightDaemon);
  oldMessageListeners.forEach(Editor.highlightDaemon.on.bind(Editor.highlightDaemon, 'message'));
};
Editor._spawnHighlightDaemon();
Editor._highlightBuckets = 0;

Editor.prototype._initHighlighting = function () {
  var self = this;

  Editor._highlightBuckets++;
  self.data.markup = {lines: [''], revision: 0, bucket: Editor._highlightBuckets};
  ...
  self.on('change', function (change, start, end) {
    ...
    textUtil.spliceLines(dataMarkup.lines,
      ...
      textUtil._getLines(markup.escape(change)));
    dataMarkup.revision++;
    self.screen.render();

    Editor.highlightDaemon.send({
      text: self.text(),
  ...
  Editor.highlightDaemon.on('message', function (data) {
