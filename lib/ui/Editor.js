...
Editor.prototype.open = Promise.method(function (path, line) {
  var self = this;
  var initialLine;
  
  path = util.resolvePath(path);
  initialLine = Math.max(parseInt(line) || 0, 1);
  ...
  return self.ready // ensures textBuf path-changed is triggered in _initHandlers
    ...
    .then(function (contents) { if (contents && contents.lines) initialLine = Math.min(initialLine, contents.lines.length); })
    .then(function () { self.selection.setHeadPosition(new Point(initialLine - 1, 0)); })
    .catch(function (err) {
