...
Editor.prototype.open = Promise.method(function (path) {
  ...
  return self.ready // ensures textBuf path-changed is triggered in _initHandlers
    ...
    .catch(function (err) {
      ...
      if (err.code === 'ENOENT') {
        extractPath = util.extractCoordsFromPath(path);
        
        if (extractPath.extractable) {
          path = extractPath.cleanPath;
          
          initialCursor.y = extractPath.coords[0] || 1;
          initialCursor.x = extractPath.coords[1] || 1;
    ...
    .tap(function () { return self.textBuf.setPath(path); })
    .then(function (dontLoad) { if (!dontLoad) return self.textBuf.load(); })
    .then(function (content) { return self._clipCursorToContent(initialCursor, content); })
    .then(function (cursor) {
      self.selection.setHeadPosition(new Point(initialCursor.y, initialCursor.x));
...
Editor.prototype._clipCursorToContent = function (cursor, content) {
  if (content && content.lines) {
    cursor.y = Math.min(cursor.y, content.lines.length) - 1;
    cursor.x = Math.min(cursor.x, content.lines[cursor.y].length) - 1;
  }
