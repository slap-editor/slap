    ...
    if (mouseData.action === 'wheeldown' || mouseData.action === 'wheelup') {
      ...
      if (mouseData.action === 'mousedown') {
        ...
        if (lastClick && lastClick.isEqual(mouse)) {
          ...
          selection.setRange(new Range(new Point(mouse.row, startX), new Point(mouse.row, endX)));
        } else {
          if (!self.data.mouseDown) selection.clearTail();
          self.data.mouseDown = true;
        }
      }
      if (self.data.mouseDown) {
        selection.setHeadPosition(mouse);
        selection.plantTail();
      }
