...
function Header (opts) {
  ...
  self.rightContent = new BaseWidget(_({
      parent: self,
      tags: true,
      shrink: true,
      style: self.options.style
  ...
  self.messageContent = new BaseWidget(_({
      parent: self,
      tags: true,
      shrink: true,
      style: self.options.style
...
Header.prototype._initHandlers = function () {
  ...
  self.helpButton.on('press', function () { Slap.global.help(); });
...
Header.prototype.render = function () {
  ...
  if (editor) {
    ...
    var markupPath = originalPath
      ? blessed.escape(path.relative(Slap.global.fileBrowser.cwd, originalPath))
      : "new file";
    ...
    if (!slap.insertMode()) right.unshift(util.markup("OVR", style.overwrite));
  }
