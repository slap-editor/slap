...
Pane.prototype._initHandlers = function () {
  ...
  ['path-changed', 'changed'].forEach(function (evt) {
    editor.textBuf.on(evt, function () { self.screen.header.render(); });
  });
  editor.on('insertMode', function () { self.screen.header.render(); });
