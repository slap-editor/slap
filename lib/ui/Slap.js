...
function Slap (opts) {
  ...
  self.fileBrowser = new FileBrowser(_({
    ...
    left: 0,
    bottom: (self.header.headerPosition === 'top' ? 0 : 1)
...
Slap.prototype.save = Promise.method(function (givenPath) {
  var self = this;
  givenPath = givenPath ? Slap.normalizePath(givenPath) : self.path();
  if (!givenPath) {
    self.header.message("couldn't save, no filename passed", 'warning');
    return;
  }
