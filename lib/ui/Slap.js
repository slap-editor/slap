...
Slap.prototype._stopKeyPropagation = function () {
  ...
  var self = this;
  self.lockKeys = true;
  return util.nextTick().then(function () { self.lockKeys = false; });
};
...
Slap.prototype._initHandlers = function () {
  ...
  Promise.all([
    Promise.promisify(npm.load, npm)({depth: 0})
      .then(function () { return Promise.promisify(npm.commands.ls, npm)([], true); })
      .spread(function (ls) {
        var packages = ls.dependencies;
        return Object.keys(packages).filter(function (pkg) {
          return (packages[pkg].keywords || []).indexOf('slap-plugin') !== -1;
        });
      }),
    self.ready
    ...
    .then(function (plugins) {
      return [].concat.apply([], plugins).reduce(function (result, pkg) {
        try {
          result[pkg] = require(pkg)(self);
          logger.verbose("loaded plugin:", pkg);
        } catch (e) {
