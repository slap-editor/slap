...
Slap.prototype.quit = function () {
  ...
  this.program.input.push('');
  this.program.disableMouse();
  // this.program.showCursor();
  // highlightClient.disconnect();

  // var transports = logger.transports;
  var transports = {};
  // logger.warn(Object.keys(logger));
  setTimeout(function () {
    exit(0, [process.stdout, process.stderr].concat(Object.keys(transports).reduce(function (streams, name) {
      var stream = transports[name]._stream;
