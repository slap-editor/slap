...
Slap.prototype.quit = function () {
  ...
  if (Editor.highlightClient) quit = quit
    ...
    .then(function (client) { client.dontRespawn = true; return self.panes; })
    .map(function (pane) { pane.editor.detach(); });
...
Slap.prototype._initHandlers = function () {
  ...
  self.on('keypress', function (ch, key) {
    var binding = BaseWidget.prototype.resolveBinding.call(self, key);
    ...
    if (binding) {
      ...
      var focused = {parent: self.focused}, focusedBinding;
      while ((focused = focused.parent) && !(focusedBinding = BaseWidget.prototype.resolveBinding.call(focused, key)));
      if (focusedBinding) bindingInfo = focusedBinding + " on " + util.typeOf(focused);
    }
    if (bindingInfo) {
      util.logger.silly(logLine + " (bound to " + bindingInfo + ")");
    } else {
