...
var Interpolator = require('regexp-capture-interpolation');
...
FindForm.prototype._initHandlers = function () {
  ...
  var interpolateReplacement = function (match, replacement) {
    return Interpolator.interpolate(match.concat([match.index,match.input]), replacement);
  };
  ...
  var findOrReplace = function (pattern, direction, replacement) {
    ...
    if (!matches.some(function (match) {
      ...
      if (cmp === direction) {
        ...
        if (replacement) {
          editor.textBuf.setTextInRange(editor.selection.getRange(), interpolateReplacement(match.match, replacement));
        }
        return true;
      ...
      if (replacement) {
        editor.textBuf.setTextInRange(editor.selection.getRange(), interpolateReplacement(matches[0].match, replacement));
      }
    }
    editor._updateContent();
  };
