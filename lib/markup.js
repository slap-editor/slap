...
markup.highlight = function (text, language, style) {
  if (language === false) return markup.escape(text);
  ...
  var $ = cheerio.load(markup.escape(highlighted.value)); // Assumes hljs output doesn't use curlies
